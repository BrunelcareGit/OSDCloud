<#
.SYNOPSIS
Remediate script about user profile deletion for 'defaultuser0' account
...
17-05-2024		Akos Bakos		Added local user account deletion

#>

$Global:Transcript = "$((Get-Date).ToString('yyyy-MM-dd-HHmmss'))-Remediate-RemoveUserProfile.log"
      ...
      If ($accountName.ToLower() -Eq $UserName.ToLower() -Or 
        ...
        If ($Force -Or $PSCmdlet.ShouldProcess($account)) {
          Try {
            ...
            Write-Host "Trying to remove defaultuser0 user account"
...
try {
  Write-Host "Trying to clean defaultuser0 profile"
  StartScript -UserName "defaultuser0" -Force
}
catch {
